    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* header */}
            <div className="flex items-center justify-between pb-4 mb-4 mt-2">
            ...
            {/* size buttons */}
            <div className="flex flex-col gap-3 mb-auto mx-auto w-[280px]">
              {singleProduct?.sizes?.map((item, index) => (
                <button
                  key={index}
                  onClick={() => setLocalSize(item)}
                  className={` w-full h-14 text-sm font-semibold tracking-wide uppercase flex items-center px-5 transition hover:border-black
              ...
              {localSize && (
                <button
                  ...
                  className="bg-black text-white uppercase py-4 px-3 text-md mt-5"
                >
