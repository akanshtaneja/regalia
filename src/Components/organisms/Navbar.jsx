import React, {useState, useEffect,  memo,  lazy,useCallback,useRef} from "react";
import { Link, NavLink, useNavigate } from "react-router-dom";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faShoppingCart } from "@fortawesome/free-solid-svg-icons";
import { useCart } from "../../context/CartContext";
const MiniCart = lazy(() => import("../../pages/MiniCart"));
...
const Navbar = ({ user, setUser }) => {
  ...
  const searchProduct = (value) => {
    navigate({
      pathname: "/products",
      search: `?category=All&sort=&page=1&min=0&max=50000&q=${value}`,
    });
  };

  const debouncedSearchRef = useRef(debounce(searchProduct, 500));
  ...
  return (
    <>
      <div className="bg-white border-b shadow-sm sticky top-0 z-50 ">
        <div className="max-w-5xl mx-auto px-4 md:px-10 py-6 flex justify-between items-center">
          {/* Logo / location */}
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="hidden md:flex relative">
              ...
              <input
                ...
                className=" border border-gray-300 rounded-full pl-10 pr-3 py-2  w-[220px] lg:w-[260px]  text-sm focus:border-black"
              />
            </div>
            ...
            {user ? (
              ...
              >
                ...
                {/* desktop dropdown */}
                {isDropdownOpen && (
                <div className="absolute top-full mt-3 bg-white border rounded-xl shadow-lg w-52">
                    <MenuItems
                      user={user}
      ...
      {isMobileSearchOpen && (
        <div className="md:hidden px-4 py-3 border-b bg-white">
          <div className="relative">
            <CiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600 pointer-events-none" />
            <input
              type="text"
              placeholder="Search Jewellery"
              onChange={(e) => handleSearch(e.target.value)}
              className="w-full border border-gray-300 rounded-full pl-10 pr-10 py-2.5 text-sm focus:border-black"
            />
            <button
